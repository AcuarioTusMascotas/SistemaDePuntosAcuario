...
function guardarCliente() {
    ...
    if (!name || !phone || isNaN(purchase) || purchase <= 0) {
    ...
    if (db.clientes.some(c => c.phone === phone)) {
...
function agregarCompra() {
    ...
    alert(`➕ $${monto.toFixed(2)} agregados\nTotal: $${cliente.totalPurchases.toFixed(2)}\nPuntos: ${cliente.points.toFixed(2)}`);
...
function exportarTablero() {
    ...
    title.textContent = '🐠 TOP 5 CLIENTES 🐠';
...
function mostrarClientesPorPuntos(minPuntos, maxPuntos, categoria) {
    ...
    if (categoria === 'gold') {
        titulo.innerHTML = `🥇 CLIENTES ORO (100,000+ PUNTOS) <small>${clientesFiltrados.length} clientes</small>`;
    } else if (categoria === 'silver') {
        titulo.innerHTML = `🥈 CLIENTES PLATA (30,000 - 60.000 PUNTOS) <small>${clientesFiltrados.length} clientes</small>`;
    } else {
        titulo.innerHTML = `🥉 CLIENTES BRONCE (0 - 30.000 PUNTOS) <small>${clientesFiltrados.length} clientes</small>`;
    }
